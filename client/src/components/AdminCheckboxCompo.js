...
import { Button, Card, CardContent, Container, Grid, Typography, alpha, TextField, Box} from '@mui/material';
...
export default function AdminCheckboxCompo() {
  ...
  const [checkboxCompo, setcheckboxCompo] = useState('');

  const poscheckboxvalue = async () => {
    try {
      await axios.post(`${URL}/checkboxComponents`, { checkboxCompo });
      setcheckboxCompo('');
      const result = await axios(`${URL}/checkboxComponents`);
      setItems(result.data);
    } catch (error) {
  ...
  const onValueChange = (e) => {
    setcheckboxCompo(e.target.value);
  };
      ...
      <Container maxWidth="lg" sx={{marginTop:"25px" }}>
        <Grid container spacing={3}>
            ...
            >
              <CardContent>
                <Typography variant="h6" component="h2" gutterBottom>
                  Add HTML+CSS for Checkbox Component
                </Typography>
                <TextField
                ...
                }}variant="contained" onClick={poscheckboxvalue}>
                  Add
                </Button>
              </CardContent>
            </Card>
          ...
          <Grid container spacing={3} sx={{ height:"55vh", overflowY: 'scroll',marginTop: "10px",marginLeft:"0px",marginBottom:"0px",marginRight:"0px"}}>
            {items.map((item) => (
              <Grid item key={item._id} xs={12} sm={6} md={3} lg={1.98}>
                <Card
                  sx={{
                    margin: 'auto',
                    flex: '0 0 calc(32vw - (2.7em * 2))',
